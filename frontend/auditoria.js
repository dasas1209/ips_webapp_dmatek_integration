/**
 * auditoria.js
 * modulo de relatorios de auditoria industrial
 */
...
function validarIntervalo() {
    ...
    if (diffDias > 30 || diffDias <= 0) { aviso.style.display = "block"; return false; }
...
async function gerarPreview() {
    if (!validarIntervalo()) { alert("Intervalo temporal inválido. Verifique as datas."); return; }
...
function renderizarPreview(dados, iv) {
    ...
    document.getElementById("badge-periodo-preview").textContent =
        formatarData(iv.inicio) + " → " + formatarData(iv.fim);
...
function renderizarIncidentes(incidentes) {
    ...
    if (incidentes.length === 0) {
        lista.innerHTML = '<li class="incident-empty">Nenhum incidente registado neste periodo.</li>';
        return;
    }
...
function exportarPNG() {
    ...
    if (iv2) {
        const periodoTxt = "Periodo: " + formatarData(iv2.inicio) + " - " + formatarData(iv2.fim);
...
async function exportarPDF() {
    ...
    try {
        ...
        pdf.text("RELATORIO DE AUDITORIA", W / 2, 63, { align: "center" });
        ...
        pdf.setTextColor(200, 210, 230); pdf.setFontSize(10);
        pdf.text("De:  " + formatarData(iv.inicio), W / 2, 118, { align: "center" });
        pdf.text("Ate: " + formatarData(iv.fim),    W / 2, 127, { align: "center" });
        pdf.text("Gerado em: " + formatarData(new Date().toISOString()), W / 2, 136, { align: "center" });

        const kf = dadosAuditoria.kpis_frota;
        const kpisTexto = [
            ["Distancia Total", kf.distancia_total_m + " m"],
            ["Bateria Media",   kf.bateria_media_perc + "%"],
            ["Total Incidentes", String(kf.total_incidentes)],
            ["Tags Ativas",     String(kf.tags_ativas)],
        ];
        let yKpi = 155;
        pdf.setFontSize(10); pdf.setTextColor(150, 170, 220);
        pdf.text("SUMARIO EXECUTIVO", W / 2, yKpi, { align: "center" });
        yKpi += 8;
        ...
        // pagina 2: esparguete em landscape
        pdf.addPage("a4", "l");
        const Wl = 297, Hl = 210;
        pdf.setFillColor(244, 246, 251); pdf.rect(0, 0, Wl, Hl, "F");
        pdf.setTextColor(26, 31, 54); pdf.setFontSize(14); pdf.setFont("helvetica", "bold");
        pdf.text("Diagrama de Esparguete - Trajetorias", Wl / 2, 14, { align: "center" });
        pdf.setFontSize(9); pdf.setFont("helvetica", "normal"); pdf.setTextColor(100, 120, 150);
        pdf.text("De: " + formatarData(iv.inicio) + "   Ate: " + formatarData(iv.fim) + "   Cliente: " + tenant, Wl / 2, 21, { align: "center" });
        ...
        if (imagemMapaAuditoria.complete && imagemMapaAuditoria.naturalWidth > 0) {
            ...
            ctxEx.fillStyle = "#e8ecf4"; ctxEx.fillRect(0, 0, 1200, 750);
        }
        ...
        let xLeg = 12, yLeg = Hl - 5;
        pdf.setFontSize(7);
        tagsUnicas.forEach(tagId => {
            const hex = corDaTag(tagId);
            const r = parseInt(hex.slice(1,3),16), g = parseInt(hex.slice(3,5),16), b = parseInt(hex.slice(5,7),16);
            pdf.setFillColor(r, g, b); pdf.rect(xLeg, yLeg - 3, 6, 3, "F");
            pdf.setTextColor(50, 60, 80); pdf.text(tagId, xLeg + 8, yLeg);
            xLeg += 30;
            if (xLeg > Wl - 30) { xLeg = 12; yLeg -= 8; }
        });

        // pagina 3: kpis por tag
        pdf.addPage("a4", "p");
        pdf.setFillColor(244, 246, 251); pdf.rect(0, 0, W, H, "F");
        pdf.setTextColor(26, 31, 54); pdf.setFontSize(14); pdf.setFont("helvetica", "bold");
        pdf.text("KPIs por Tag", W / 2, 18, { align: "center" });
        pdf.setFontSize(9); pdf.setFont("helvetica", "normal"); pdf.setTextColor(100, 120, 150);
        pdf.text(`Periodo: ${formatarData(iv.inicio)} - ${formatarData(iv.fim)}`, W / 2, 25, { align: "center" });

        const cabecalhos = ["Tag", "Dist. (m)", "OEE (%)", "Bat. Min (%)", "Ocioso (min)", "Alertas"];
        const larguras   = [30, 32, 28, 35, 38, 28];
        let xTbl = 15, yTbl = 35;

        pdf.setFillColor(26, 31, 54); pdf.rect(xTbl, yTbl, W - 30, 8, "F");
        pdf.setTextColor(255, 255, 255); pdf.setFontSize(8); pdf.setFont("helvetica", "bold");
        let xH = xTbl + 2;
        cabecalhos.forEach((h, i) => { pdf.text(h, xH, yTbl + 5.5); xH += larguras[i]; });

        yTbl += 8; pdf.setFont("helvetica", "normal");
        dadosAuditoria.kpis_por_tag.forEach((k, idx) => {
            ...
            const linha = [k.tag_id, String(k.distancia_m), String(k.oee_perc), String(k.bateria_min_perc), String(k.tempo_ocioso_min), k.num_alertas > 0 ? k.num_alertas + " alerta(s)" : "OK"];
            let xL = xTbl + 2;
            linha.forEach((v, i) => {
                if (i === 5 && k.num_alertas > 0) pdf.setTextColor(180, 30, 30);
                pdf.text(v, xL, yTbl + 5); pdf.setTextColor(40, 50, 70); xL += larguras[i];
            });
            yTbl += 7;
        });

        // pagina 4: log de incidentes
        pdf.addPage("a4", "p");
        pdf.setFillColor(244, 246, 251); pdf.rect(0, 0, W, H, "F");
        pdf.setTextColor(26, 31, 54); pdf.setFontSize(14); pdf.setFont("helvetica", "bold");
        ...
        pdf.setFillColor(180, 30, 30); pdf.rect(xInc, yInc, W - 20, 8, "F");
        pdf.setTextColor(255, 255, 255); pdf.setFontSize(8); pdf.setFont("helvetica", "bold");
        let xHI = xInc + 2;
        cabInc.forEach((h, i) => { pdf.text(h, xHI, yInc + 5.5); xHI += largInc[i]; });

        yInc += 8; pdf.setFont("helvetica", "normal");
        if (dadosAuditoria.incidentes.length === 0) {
            pdf.setTextColor(100, 150, 100); pdf.setFontSize(11);
            pdf.text("Nenhum incidente registado neste periodo.", W / 2, yInc + 20, { align: "center" });
        } else {
            dadosAuditoria.incidentes.forEach((inc, idx) => {
                if (yInc > H - 20) { pdf.addPage("a4", "p"); yInc = 20; }
                ...
                linhaInc.forEach((v, i) => {
                    ...
                    pdf.text(v, xLI, yInc + 5); pdf.setTextColor(40, 50, 70); xLI += largInc[i];
                });
                yInc += 7;
            });
        }
        ...
        for (let p = 1; p <= totalPag; p++) {
            pdf.setPage(p);
            pdf.setFontSize(7); pdf.setTextColor(150, 160, 180);
            const [Wp, Hp] = dimPag(p);
            pdf.text(`Metric4 RTLS - Auditoria Industrial | Pag. ${p}/${totalPag}`, Wp / 2, Hp - 5, { align: "center" });
        }
